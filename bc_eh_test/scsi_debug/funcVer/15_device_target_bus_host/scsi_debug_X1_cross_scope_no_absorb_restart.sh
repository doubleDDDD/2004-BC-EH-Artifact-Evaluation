...
readonly CASE_ID="X1_cross_scope_no_absorb_restart"
readonly RUN_DIR="${BC_EH_SCSI_DEBUG_ROOT}/funcVer/15_device_target_bus_host/${CASE_ID}"
readonly WAIT_SECS="${BC_EH_CROSS_SCOPE_WAIT_SECS:-30}"
readonly POLL_INTERVAL_SECS="${BC_EH_CROSS_SCOPE_POLL_INTERVAL_SECS:-1}"
...
main()
{
...

    {
        ...
        printf 'expect_log_2=restart EH from pending fault\n'
        printf 'host=%s\n' "${scsi_debug_host}"
        printf 'A=%s /dev/%s\n' "${A_scsi_id}" "${A_block}"
        printf 'B=%s /dev/%s\n' "${B_scsi_id}" "${B_block}"
        printf 'C=%s /dev/%s\n' "${C_scsi_id}" "${C_block}"
    ...
    (
        ...
        bc_eh_log "target reset phase observed, inject cross-scope late fault into C"
