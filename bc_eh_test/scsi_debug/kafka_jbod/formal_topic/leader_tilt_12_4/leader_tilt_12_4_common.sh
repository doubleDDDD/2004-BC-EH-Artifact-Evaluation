...
LEADER_TILT_12_4_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FORMAL_TOPIC_OUTPUT_DIR="${FORMAL_TOPIC_OUTPUT_DIR:-${LEADER_TILT_12_4_SCRIPT_DIR}/output}"
# shellcheck source=/dev/null
. "${LEADER_TILT_12_4_SCRIPT_DIR}/../formal_topic_common.sh"

LT12_TARGET_LOG_DIR="${LT12_TARGET_LOG_DIR:-/data/kafka-1/kafka-logs}"
LT12_SIBLING_LOG_DIR_A="${LT12_SIBLING_LOG_DIR_A:-/data/kafka-2/kafka-logs}"
LT12_SIBLING_LOG_DIR_B="${LT12_SIBLING_LOG_DIR_B:-/data/kafka-3/kafka-logs}"

LT12_TARGET_PARTITION_COUNT="${LT12_TARGET_PARTITION_COUNT:-16}"
LT12_TARGET_LEADER_COUNT="${LT12_TARGET_LEADER_COUNT:-12}"
LT12_SIBLING_PARTITION_COUNT="${LT12_SIBLING_PARTITION_COUNT:-16}"
LT12_SIBLING_LEADER_COUNT="${LT12_SIBLING_LEADER_COUNT:-2}"

LT12_REASSIGN_VERIFY_ATTEMPTS="${LT12_REASSIGN_VERIFY_ATTEMPTS:-90}"
LT12_REASSIGN_VERIFY_INTERVAL_SECS="${LT12_REASSIGN_VERIFY_INTERVAL_SECS:-2}"
LT12_LAYOUT_VERIFY_ATTEMPTS="${LT12_LAYOUT_VERIFY_ATTEMPTS:-30}"
LT12_LAYOUT_VERIFY_INTERVAL_SECS="${LT12_LAYOUT_VERIFY_INTERVAL_SECS:-2}"

lt12_output_prefix() {
    printf '%s/%s.leader_tilt_12_4' "${FORMAL_TOPIC_OUTPUT_DIR}" "$(ft_topic_safe_name)"
}
...
lt12_reassignment_file() {
    printf '%s.reassignment.json\n' "$(lt12_output_prefix)"
}

lt12_preferred_election_file() {
    printf '%s.preferred_election.json\n' "$(lt12_output_prefix)"
}

lt12_layout_plan_file() {
    printf '%s.layout_plan.json\n' "$(lt12_output_prefix)"
}

lt12_layout_plan_summary_file() {
    printf '%s.layout_plan.txt\n' "$(lt12_output_prefix)"
}

lt12_actual_describe_file() {
    printf '%s.actual.describe.txt\n' "$(lt12_output_prefix)"
}

lt12_actual_logdirs_file() {
    printf '%s.actual.logdirs.json\n' "$(lt12_output_prefix)"
}

lt12_actual_summary_file() {
    printf '%s.actual.summary.txt\n' "$(lt12_output_prefix)"
}

lt12_reassign_verify_file() {
...
lt12_generate_plan_files() {
    ...
    assignment_file="$(lt12_replica_assignment_file)"
    reassignment_file="$(lt12_reassignment_file)"
    preferred_election_file="$(lt12_preferred_election_file)"
    layout_plan_file="$(lt12_layout_plan_file)"
    layout_plan_summary_file="$(lt12_layout_plan_summary_file)"
    ...
    python3 - \
        ...
        "${LT12_TARGET_LOG_DIR}" \
        "${LT12_SIBLING_LOG_DIR_A}" \
        "${LT12_SIBLING_LOG_DIR_B}" \
        "${LT12_TARGET_PARTITION_COUNT}" \
        "${LT12_TARGET_LEADER_COUNT}" \
        "${LT12_SIBLING_PARTITION_COUNT}" \
        "${LT12_SIBLING_LEADER_COUNT}" \
        "${assignment_file}" \
        "${reassignment_file}" \
...
if replication_factor != 3:
    raise SystemExit("[kafka-formal-topic][error] leader_tilt_12_4 currently requires replication factor = 3")
if partition_count != 48:
    raise SystemExit("[kafka-formal-topic][error] leader_tilt_12_4 currently requires partition count = 48")
if target_broker_id != 1:
    raise SystemExit("[kafka-formal-topic][error] leader_tilt_12_4 currently requires target broker = 1")
if target_partition_count != 16 or sibling_partition_count != 16:
    raise SystemExit("[kafka-formal-topic][error] leader_tilt_12_4 currently requires 16 replicas per broker-1 log.dir")
if target_leader_count != 12 or sibling_leader_count != 2:
    raise SystemExit("[kafka-formal-topic][error] leader_tilt_12_4 currently requires leader counts 12 / 2 / 2")
