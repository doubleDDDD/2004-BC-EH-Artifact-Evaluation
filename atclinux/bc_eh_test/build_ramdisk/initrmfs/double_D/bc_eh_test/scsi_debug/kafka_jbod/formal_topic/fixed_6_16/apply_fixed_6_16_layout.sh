...
wait_for_reassignment() {
    ...
    verify_file="$(fx616_reassign_verify_file)"

    for attempt in $(seq 1 "${FX616_REASSIGN_VERIFY_ATTEMPTS}"); do
        output="$("${KAFKA_BIN_DIR}/kafka-reassign-partitions.sh" \
            --bootstrap-server "${bootstrap_server}" \
            --verify \
            --reassignment-json-file "$(fx616_reassignment_file)" 2>&1 || true)"
        printf '%s\n' "${output}" > "${verify_file}"
        ...
        if printf '%s\n' "${output}" | grep -qi "still in progress"; then
            sleep "${FX616_REASSIGN_VERIFY_INTERVAL_SECS}"
            continue
        fi
        return 0
    done
...
run_preferred_leader_election() {
    ...
    if [[ -x "${KAFKA_BIN_DIR}/kafka-leader-election.sh" ]]; then
        "${KAFKA_BIN_DIR}/kafka-leader-election.sh" \
            ...
            --path-to-json-file "$(fx616_preferred_election_file)"
        return 0
    fi
...
wait_for_exact_layout() {
    ...
    for attempt in $(seq 1 "${FX616_LAYOUT_VERIFY_ATTEMPTS}"); do
        if BOOTSTRAP_SERVER="${bootstrap_server}" bash "${SCRIPT_DIR}/check_fixed_6_16_layout.sh" >/dev/null 2>&1; then
            return 0
        fi
        sleep "${FX616_LAYOUT_VERIFY_INTERVAL_SECS}"
    done

    BOOTSTRAP_SERVER="${bootstrap_server}" bash "${SCRIPT_DIR}/check_fixed_6_16_layout.sh"
    ft_die "fixed_6_16 exact layout was not observed within the expected time"
}
...
main() {
    ...
    command -v python3 >/dev/null 2>&1 || ft_die "python3 is required for fixed_6_16 planning"
    ...
    if topic_exists "${bootstrap_server}"; then
        ft_log "Formal topic ${FORMAL_TOPIC_NAME} already exists; reusing it and converging to fixed_6_16"
        validate_existing_topic_shape "${bootstrap_server}"
    else
        assignment="$(tr -d '\n' < "$(fx616_replica_assignment_file)")"
        ft_log "Creating formal topic ${FORMAL_TOPIC_NAME} with deterministic replica ordering for fixed_6_16"
        "${KAFKA_BIN_DIR}/kafka-topics.sh" \
            --create \
            --topic "${FORMAL_TOPIC_NAME}" \
            --bootstrap-server "${bootstrap_server}" \
    ...
    ft_log "Executing deterministic replica/log.dir reassignment for fixed_6_16"
    "${KAFKA_BIN_DIR}/kafka-reassign-partitions.sh" \
        --bootstrap-server "${bootstrap_server}" \
        --execute \
        --reassignment-json-file "$(fx616_reassignment_file)"
    ...
    ft_log "Waiting for exact fixed_6_16 layout to converge"
    wait_for_exact_layout "${bootstrap_server}"

    ft_log "fixed_6_16 layout apply finished"
    ft_log "Plan summary: $(fx616_layout_plan_summary_file)"
    ft_log "Exact-layout summary: $(fx616_actual_summary_file)"
}
