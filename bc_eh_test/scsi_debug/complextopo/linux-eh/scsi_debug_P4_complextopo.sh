...
#   echo 'device fail 1' > /sys/kernel/debug/scsi_debug/<host_no>:0:0:0/validate_after_reset
#   echo 'clear' > /sys/kernel/debug/scsi_debug/<host_no>:0:0:1/validate_after_reset
#   echo 'device fail 1' > /sys/kernel/debug/scsi_debug/<host_no>:0:0:1/validate_after_reset
...
#   cd /double_D && ./fio --filename=/dev/<C_block> --ioengine=libaio --direct=1 --iodepth=64 --rw=randread --bs=4k --numjobs=1 --thread --size=100% --time_based --runtime=120 --group_reporting --name=randread_4k_C
# - A/B 上读写命令 0x28 / 0x2a 持续 IO timeout
# - A/B 上 abort 失败
# - device reset 成功
# - 随后的 TUR 验证失败（validate_after_reset: device fail 1）
# - target reset 成功，随后 I/O 恢复
# - 恢复链：D+ / V- -> T+

export BC_EH_PROFILE=linux-eh
export BC_EH_MODE_VALUE=host
export BC_EH_CASE=P4
