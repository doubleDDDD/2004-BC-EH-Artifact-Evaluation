...
main() {
    ...
    command -v python3 >/dev/null 2>&1 || ft_die "python3 is required for leader_tilt_12_4 layout checks"
    ...
    describe_file="$(lt12_actual_describe_file)"
    ...
    logdirs_file="$(lt12_actual_logdirs_file)"
    printf '%s\n' "${logdirs_output}" > "${logdirs_file}"

    summary_file="$(lt12_actual_summary_file)"
    plan_file="$(lt12_layout_plan_file)"
