...
#   echo 'clear' > /sys/kernel/debug/scsi_debug/<host_no>:0:0:0/validate_after_reset
#   echo 'host fail 1' > /sys/kernel/debug/scsi_debug/<host_no>:0:0:0/validate_after_reset
...
#   cd /double_D && ./fio --filename=/dev/<A_block> --ioengine=libaio --direct=1 --iodepth=64 --rw=randread --bs=4k --numjobs=1 --thread --size=100% --time_based --runtime=30 --group_reporting --name=randread_4k_A
...
#   cd /double_D && ./fio --filename=/dev/<C_block> --ioengine=libaio --direct=1 --iodepth=64 --rw=randread --bs=4k --numjobs=1 --thread --size=100% --time_based --runtime=30 --group_reporting --name=randread_4k_C
#   cd /double_D && ./fio --filename=/dev/<D_block> --ioengine=libaio --direct=1 --iodepth=64 --rw=randread --bs=4k --numjobs=1 --thread --size=100% --time_based --runtime=30 --group_reporting --name=randread_4k_D
...
# - host reset 成功
# - 随后的 host 级 TUR 验证失败（validate_after_reset: host fail 1）
# - 恢复链：D- -> T- -> B- -> H+ / V-
