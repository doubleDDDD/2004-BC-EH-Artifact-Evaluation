...
# - active 节点：A、B
# - idle 节点：C、D、E、F、G、H
# - 实际仅对 A（channel0/target0/lun0）注入故障，B 作为同 target 下的健康 active 兄弟
...
#   cd /double_D && ./fio --filename=/dev/<B_block> --ioengine=libaio --direct=1 --iodepth=64 --rw=randread --bs=4k --numjobs=1 --thread --size=100% --time_based --runtime=120 --group_reporting --name=randread_4k_B
# - A 上读写命令 0x28 / 0x2a 持续 IO timeout
# - A 上 abort 失败
# - device reset 成功，随后 I/O 恢复
# - 恢复链：D+
