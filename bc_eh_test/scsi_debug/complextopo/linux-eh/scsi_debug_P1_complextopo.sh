...
# Fault scenario:
# - Paper case: P1, LUN-scope transient fault
...
# - Inject faults only into A (channel0/target0/lun0); B is the healthy active sibling under the same target
...
#   fio --filename=/dev/<B_block> --ioengine=libaio --direct=1 --iodepth=64 --rw=randread --bs=4k --numjobs=1 --thread --size=100% --time_based --runtime=120 --group_reporting --name=randread_4k_B
# - A has persistent IO timeout on read/write commands 0x28 / 0x2a
# - A abort fails
# - device reset succeeds, then I/O recovers
# - Recovery chain: D+
